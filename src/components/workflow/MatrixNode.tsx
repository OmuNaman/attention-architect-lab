...
export function MatrixNode({ data }: MatrixNodeProps) {
  ...
  return (
    <Card className={`min-w-[280px] transition-colors duration-300 ${
      isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300' 
    } shadow-xl rounded-lg`}> {/* Added rounded-lg and increased shadow */}
      <div className="p-4">
        <div className="text-center mb-3">
          <h3 className={`font-semibold text-lg mb-1 ${isDark ? 'text-slate-100' : 'text-slate-800'}`}>{data.label}</h3>
          <p className={`text-sm ${
